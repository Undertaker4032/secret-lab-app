...
<div class="bg-rms-cod-gray border border-rms-mine-shaft hover:border-rms-white/30 hover:bg-rms-mine-shaft/50 transition-all duration-300 p-6 group">
    ...
    <div class="flex items-center gap-4 flex-1 min-w-0">
      {#if employee.profile_picture}
        <img
          ...
          alt={employee.name}
          class="h-14 w-14 rounded-full object-cover border-2 border-rms-mine-shaft group-hover:border-rms-white/30 transition-colors"
        />
      {:else}
        <div class="h-14 w-14 rounded-full bg-rms-mine-shaft flex items-center justify-center border border-rms-dove-gray group-hover:border-rms-white/30 transition-colors">
      ...
      <div class="flex-1 min-w-0">
        <div class="flex flex-wrap items-start gap-3 mb-2">
          ...
          {#if !employee.is_active}
            <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-gray-900/40 text-gray-300 border border-gray-600/50 whitespace-nowrap flex-shrink-0">
              Неактивен
            </span>
          {/if}
        </div>
  ...
  {#if employee.cluster}
    <div class="mt-3 pt-3 border-t border-rms-mine-shaft">
      <div class="flex items-center gap-2 text-sm text-rms-nobel">
        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
